...
if (isset($_POST["id_prod"]) && isset($_POST["id_usu"]) && strlen(trim($_POST["id_prod"]))>0 && strlen(trim($_POST["id_usu"]))>0){
    ...
    if (is_numeric($_POST["id_prod"]) && is_numeric($_POST["id_usu"])){
        ...
        if ($_POST["id_usu"]==$idSes){
            $con=conexUsu();
            $prod=$_POST["id_prod"];
            $sql="SELECT ID_PROD,ID_USU,ID_COMPRADOR,ID_RESERVA FROM productos where ID_PROD=$prod";
            $fila = $con->query($sql)->fetch_assoc();

            if ($fila){

                if($fila["ID_COMPRADOR"]==null && $fila["ID_USU"]!=$idSes && $fila["ID_RESERVA"]==null){

                    $insert="INSERT INTO reservas(ID_PROD, ID_USU) VALUES($prod,$idSes)";
